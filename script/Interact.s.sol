...
contract ClaimAirdrop is Script {
    ...
    // This should private because we don't want to expose the signature
    bytes SIGNATURE =
    ...
    function splitSignature(bytes memory sig) public pure returns (uint8 v, bytes32 r, bytes32 s) {
        ...
        // 65 corresponds to the length of the signature in bytes (32 bytes for r, 32 bytes for s, and 1 byte for the recovery parameter).
        if (sig.length != 65) {
